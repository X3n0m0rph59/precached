...
language. It tries to improve system performance by loading
data that will most likely be referenced soon from slow mass storage
like spinning hard disks, into the system's RAM.
...
Precached is in a production-ready state, we released version 1.0 of
precached on 2018-03-04!
...
The daemon proper is extended by "plugins" and "hooks", which deliver
...
* Hooks may not be disabled, Plugins can be disabled dynamically via the configuration file
...
* The plugin interface is designed for implementation of additional functionality, possibly
...
strictly event based. This allows us to implement an online prefetching
mechanism, and leads to a much better software design than the polling
variant would have been. The `Proc Connector` is implemented
in the crate `procmon-sys` and is used by the crate `procmon`.
...
_Application load times (in seconds, M:SS) on first run after bootup
